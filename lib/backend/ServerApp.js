...
const checkQuery =
  "SELECT COUNT(*) AS entryCount FROM DepartmentEntryLinks WHERE DepartmentName = ? AND ProductCode = ?";
...
db.query(
  ...
  (checkErr, checkResults) => {
    if (checkErr) {
      ...
      if (entryCount === 0) {
        // Entry does not exist, insert a new entry into Entries table
        ...
        db.query(
          ...
          (insertEntryErr, insertEntryResults) => {
            if (insertEntryErr) {
              ...
              // Entry inserted successfully, now insert into DepartmentEntryLinks
